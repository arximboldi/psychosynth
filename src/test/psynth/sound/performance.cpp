...
#include <psynth/sound/pixel.hpp>
#include <psynth/sound/planar_pixel_iterator.hpp>
#include <psynth/sound/planar_pixel_reference.hpp>
#include <psynth/sound/iterator_from_2d.hpp>
#include <psynth/sound/step_iterator.hpp>
#include <psynth/sound/stereo.hpp>
#include <psynth/sound/image_view.hpp>
#include <psynth/sound/image.hpp>
...
#define STEREO_VIEW(T) image_view<memory_based_2d_locator<memory_based_step_iterator<pixel<T,stereo_layout_t>*> > >
#define BGR_VIEW(T) image_view<memory_based_2d_locator<memory_based_step_iterator<pixel<T,bgr_layout_t>*> > >
#define STEREO_PLANAR_VIEW(T) image_view<memory_based_2d_locator<memory_based_step_iterator<planar_pixel_iterator<T*,stereo_t> > > >
...
struct fill_nongil_t<STEREO_VIEW(T), P> {
...
struct fill_nongil_t<STEREO_VIEW(T1), pixel<T2,bgr_layout_t> > {
...
template <typename T1, typename T2>
struct fill_nongil_t<STEREO_PLANAR_VIEW(T1), pixel<T2,stereo_layout_t> > {
    typedef STEREO_PLANAR_VIEW(T1) View;
    typedef pixel<T2,stereo_layout_t> P;
    View _v;
    P _p;
    fill_nongil_t(const View& v_in,const P& p_in) : _v(v_in), _p(p_in) {}
    void operator()() const {
...
template <typename T1, typename T2>
struct fill_nongil_t<STEREO_PLANAR_VIEW(T1), pixel<T2,bgr_layout_t> > {
    typedef STEREO_PLANAR_VIEW(T1) View;
    typedef pixel<T2,bgr_layout_t> P;
    View _v;
    P _p;
...
template <typename T>
struct stereo_fr_t {
    void operator()(pixel<T,stereo_layout_t>& p) const {p[0]=0;p[1]=1;p[2]=2;}
    void operator()(const planar_pixel_reference<T&,stereo_t>& p) const {p[0]=0;p[1]=1;p[2]=2;}
};
...
struct for_each_nongil_t<STEREO_VIEW(T), stereo_fr_t<T2> > {
    typedef STEREO_VIEW(T) View;
...
template <typename T1, typename T2>
struct for_each_nongil_t<STEREO_PLANAR_VIEW(T1), stereo_fr_t<T2> > {
    typedef STEREO_PLANAR_VIEW(T1) View;
    typedef stereo_fr_t<T2> F;
    View _v;
    F _f;
...
template <typename T1, typename T2>
struct copy_nongil_t<STEREO_VIEW(T1),STEREO_VIEW(T2)> {
...
template <typename T1, typename T2>
struct copy_nongil_t<STEREO_VIEW(T1),BGR_VIEW(T2)> {
    typedef STEREO_VIEW(T1) View1;
...
template <typename T1, typename T2>
struct copy_nongil_t<STEREO_PLANAR_VIEW(T1),STEREO_PLANAR_VIEW(T2)> {
...
template <typename T1, typename T2>
struct copy_nongil_t<STEREO_VIEW(T1),STEREO_PLANAR_VIEW(T2)> {
    typedef STEREO_VIEW(T1) View1;
    typedef STEREO_PLANAR_VIEW(T2) View2;
    View1 _v1;
    View2 _v2;
...
template <typename T1, typename T2>
struct copy_nongil_t<STEREO_PLANAR_VIEW(T1),STEREO_VIEW(T2)> {
    typedef STEREO_PLANAR_VIEW(T1) View1;
    typedef STEREO_VIEW(T2) View2;
    View1 _v1;
    View2 _v2;
...
struct bgr_to_stereo_t {
    pixel<T,stereo_layout_t> operator()(const Pixel& p) const {
        return pixel<T,stereo_layout_t>(T(get_color(p,blue_t())*0.1f),
...
struct transform_nongil_t<STEREO_VIEW(T1),STEREO_VIEW(T2),F> {
    typedef STEREO_VIEW(T1) View1;
    typedef STEREO_VIEW(T2) View2;
    View1 _v1;
    View2 _v2;
...
template <typename T1, typename T2, typename F>
struct transform_nongil_t<STEREO_PLANAR_VIEW(T1),STEREO_PLANAR_VIEW(T2),F> {
    typedef STEREO_PLANAR_VIEW(T1) View1;
    typedef STEREO_PLANAR_VIEW(T2) View2;
    View1 _v1;
    View2 _v2;
...
template <typename T1, typename T2, typename F>
struct transform_nongil_t<STEREO_VIEW(T1),STEREO_PLANAR_VIEW(T2),F> {
    typedef STEREO_VIEW(T1) View1;
    typedef STEREO_PLANAR_VIEW(T2) View2;
    View1 _v1;
    View2 _v2;
...
template <typename T1, typename T2, typename F>
struct transform_nongil_t<STEREO_PLANAR_VIEW(T1),STEREO_VIEW(T2),F> {
    typedef STEREO_PLANAR_VIEW(T1) View1;
    typedef STEREO_VIEW(T2) View2;
    View1 _v1;
    View2 _v2;
...
int main() {
    ...
    // fill()
    std::cout<<"test fill_pixels() on stereo8_image_t with stereo8_pixel_t"<<std::endl;
    test_fill<stereo8_view_t,stereo8_pixel_t>(num_trials);
    std::cout<<std::endl;

    std::cout<<"test fill_pixels() on stereo8_planar_image_t with stereo8_pixel_t"<<std::endl;
    test_fill<stereo8_planar_view_t,stereo8_pixel_t>(num_trials);
    std::cout<<std::endl;

    std::cout<<"test fill_pixels() on stereo8_image_t with bgr8_pixel_t"<<std::endl;
    test_fill<stereo8_view_t,bgr8_pixel_t>(num_trials);
    std::cout<<std::endl;

    std::cout<<"test fill_pixels() on stereo8_planar_image_t with bgr8_pixel_t"<<std::endl;
    test_fill<stereo8_planar_view_t,bgr8_pixel_t>(num_trials);
    std::cout<<std::endl;

    // for_each()
    std::cout<<"test for_each_pixel() on stereo8_image_t"<<std::endl;
    test_for_each<stereo8_view_t,stereo_fr_t<bits8> >(num_trials);
    std::cout<<std::endl;

    std::cout<<"test for_each_pixel() on stereo8_planar_image_t"<<std::endl;
    test_for_each<stereo8_planar_view_t,stereo_fr_t<bits8> >(num_trials);
    std::cout<<std::endl;

    // copy()
    std::cout<<"test copy_pixels() between stereo8_image_t and stereo8_image_t"<<std::endl;
    test_copy<stereo8_view_t,stereo8_view_t>(num_trials);
    std::cout<<std::endl;

    std::cout<<"test copy_pixels() between stereo8_image_t and bgr8_image_t"<<std::endl;
    test_copy<stereo8_view_t,bgr8_view_t>(num_trials);
    std::cout<<std::endl;

    std::cout<<"test copy_pixels() between stereo8_planar_image_t and stereo8_planar_image_t"<<std::endl;
    test_copy<stereo8_planar_view_t,stereo8_planar_view_t>(num_trials);
    std::cout<<std::endl;

    std::cout<<"test copy_pixels() between stereo8_image_t and stereo8_planar_image_t"<<std::endl;
    test_copy<stereo8_view_t,stereo8_planar_view_t>(num_trials);
    std::cout<<std::endl;

    std::cout<<"test copy_pixels() between stereo8_planar_image_t and stereo8_image_t"<<std::endl;
    test_copy<stereo8_planar_view_t,stereo8_view_t>(num_trials);
    std::cout<<std::endl;

    // transform()
    std::cout<<"test transform_pixels() between stereo8_image_t and stereo8_image_t"<<std::endl;
    test_transform<stereo8_view_t,stereo8_view_t,bgr_to_stereo_t<bits8,pixel<bits8,stereo_layout_t> > >(num_trials);
    std::cout<<std::endl;

    std::cout<<"test transform_pixels() between stereo8_planar_image_t and stereo8_planar_image_t"<<std::endl;
    test_transform<stereo8_planar_view_t,stereo8_planar_view_t,bgr_to_stereo_t<bits8,planar_pixel_reference<bits8,stereo_t> > >(num_trials);
    std::cout<<std::endl;

    std::cout<<"test transform_pixels() between stereo8_image_t and stereo8_planar_image_t"<<std::endl;
    test_transform<stereo8_view_t,stereo8_planar_view_t,bgr_to_stereo_t<bits8,pixel<bits8,stereo_layout_t> > >(num_trials);
    std::cout<<std::endl;

    std::cout<<"test transform_pixels() between stereo8_planar_image_t and stereo8_image_t"<<std::endl;
    test_transform<stereo8_planar_view_t,stereo8_view_t,bgr_to_stereo_t<bits8,planar_pixel_reference<bits8,stereo_t> > >(num_trials);
    std::cout<<std::endl;
