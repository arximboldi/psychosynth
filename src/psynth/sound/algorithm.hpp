...
/**
   ...
   Image views provide 1D iteration of their pixels via \p begin() and
   ...
   last pixel in a row it will move to the first pixel of the next
   ...
   algorithms. For example, \p copy_pixels calls \p std::copy either for
   ...
   bitwise-copiable pixels results in std::copy over unsigned char,
   which STL typically implements via \p memmove.

   As a result \p copy_pixels may result in a single call to \p memmove
...
private:
    ...
    template <typename V1, typename V2> PSYNTH_FORCEINLINE
    result_type apply(const V1& v1, const V2& v2, mpl::false_) const
    {
    ...
    // dispatch from apply overload to a function with distinct name
    template <typename V1, typename V2> PSYNTH_FORCEINLINE
    result_type apply(const V1& v1, const V2& v2, mpl::true_) const
    {
...
PSYNTH_FORCEINLINE psynth::sound::pixel<T,Cs>* 
copy (psynth::sound::pixel<T,Cs>* first, psynth::sound::pixel<T,Cs>* last, 
      psynth::sound::pixel<T,Cs>* dst)
{ 
    return (psynth::sound::pixel<T,Cs>*) std::copy (
	(unsigned char*) first,
	(unsigned char*) last,
	(unsigned char*) dst);
}
...
template<typename T, typename Cs> 
PSYNTH_FORCEINLINE psynth::sound::pixel<T,Cs>* 
copy (const psynth::sound::pixel<T,Cs>* first, const psynth::sound::pixel<T,Cs>* last, 
      psynth::sound::pixel<T,Cs>* dst)
{ 
    return (psynth::sound::pixel<T,Cs>*) std::copy(
	(unsigned char*) first,
...
psynth::sound::planar_pixel_iterator<IC2,Cs>
copy (psynth::sound::planar_pixel_iterator<IC1,Cs> first,
...
/**
   Does a copy-n. If the inputs contain image iterators, performs a
   copy at each row using the row iterators
   \ingroup CopyPixels
*/
...
// IL Models ConstPixelLocatorConcept, O Models PixelIteratorConcept
struct copier_n <iterator_from_2d<IL>,O>
{
    ...
    void operator () (iterator_from_2d<IL> src, diff_t n, O dst) const
    {
        ...
        while (n > 0) {
            ...
            detail::copy_n (src.x (), num_to_copy, dst);
            dst += num_to_copy;
            src += num_to_copy;
            n   -= num_to_copy;
        }
...
// I Models ConstPixelIteratorConcept, OL Models PixelLocatorConcept
struct copier_n<I,iterator_from_2d<OL> >
{
    ...
    void operator () (I src, diff_t n, iterator_from_2d<OL> dst) const
    {
        ...
        while (n>0) {
            ...
            diff_t num_to_copy = (n < l ? n : l);
            detail::copy_n (src, num_to_copy, dst.x ());
            dst += num_to_copy;
            src += num_to_copy;
            n   -= num_to_copy;
        }
...
/** Both source and destination ranges are delimited by image
 * iterators */
   ...
   void operator() (iterator_from_2d<IL> src, diff_t n,
		    iterator_from_2d<OL> dst) const
    {
        ...
        while (n>0) {
            diff_t l = dst.width() - dst.x_pos();
            diff_t num_to_copy= (n<l ? n : l);
            detail::copy_n (src.x(), num_to_copy, dst.x());
            dst += num_to_copy;
            src += num_to_copy;
            n   -= num_to_copy;
        }
...
PSYNTH_FORCEINLINE DstIterator
copy_with_2d_iterators(SrcIterator first, SrcIterator last,
		       DstIterator dst)
{
...
template <typename IL, typename OL>
PSYNTH_FORCEINLINE
psynth::sound::iterator_from_2d<OL> copy (psynth::sound::iterator_from_2d<IL> first,
				       psynth::sound::iterator_from_2d<IL> last,
				       psynth::sound::iterator_from_2d<OL> dst)
{
    return psynth::sound::detail::copy_with_2d_iterators (first,last,dst);
}
...
/**
   \defgroup ImageViewSTLAlgorithmsCopyAndConvertPixels
   copy_and_convert_pixels

   \ingroup ImageViewSTLAlgorithms

   \brief copies src view into dst view, color converting if
   necessary.

   Versions taking static and runtime views are provided. Versions
   taking user-defined color convered are provided.
*/
namespace detail
{
...
public:
    typedef typename
    binary_operation_obj<copy_and_convert_pixels_fn<CC> >::result_type result_type;
...
/**
   ...
   Invoked when one calls std::fill(I,I,V) with I being a
   iterator_from_2d (which is a 1D iterator over the pixels in an
   image). For contiguous images (i.e. images that have no alignment
   gap at the end of each row) it is more efficient to use the
   underlying pixel iterator that does not check for the end of
   rows. For non-contiguous images fill resolves to fill of each row
	   ...
	   psynth::sound::iterator_from_2d<IL> last, const V& val)
{
    ...
    if (first.is_1d_traversable()) {
        ...
        while (n > 0) {
            std::ptrdiff_t num_to_do = std::min<const std::ptrdiff_t>(
		n, (std::ptrdiff_t) (first.width () - first.x_pos ()));
            fill_n (first.x(), num_to_do, val);
            first += num_to_do;
            n     -= num_to_do;
        }
    }
...
{
    if (img_view.is_1d_traversable()) 
        detail::destruct_aux (img_view.begin().x(),
			      img_view.end().x(), 
			      is_planar<View>());
    else
        for (std::ptrdiff_t y = 0; y < img_view.height (); ++y)
            detail::destruct_aux (img_view.row_begin (y),
				  img_view.row_end(y),
				  is_planar<View> ());
}
...
{
    if (img_view.is_1d_traversable()) 
        detail::uninitialized_fill_aux (img_view.begin().x(),
					img_view.end().x(), 
					val, is_planar<View>());
    else {
        ...
        try {
            for (y=0; y<img_view.height(); ++y)
                detail::uninitialized_fill_aux (img_view.row_begin (y),
						img_view.row_end (y),
						val, is_planar<View>());
        } catch(...) {
...
/**
   ...
   \ingroup ImageViewSTLAlgorithms
   \brief invokes the default constructor on every pixel of an image
   view
*/
...
void default_construct_range (It first, It last)
{
	...
	first, last, typename is_pointer<It>::type());
}
...
/**
   ...
   \brief Invokes the in-place default constructor on every pixel of
   the (uninitialized) view.
   
   Does not support planar heterogeneous views.
   If an exception is thrown destructs any in-place
   default-constructed pixels
*/
template <typename View> 
void default_construct_pixels(const View& img_view)
{
    if (detail::has_trivial_pixel_constructor<
...
{
    typedef mpl::bool_<is_planar<View1>::value &&
    ...
    if (view1.is_1d_traversable() && view2.is_1d_traversable())
        detail::uninitialized_copy_aux (view1.begin().x(),
					view1.end().x(), 
					view2.begin().x(), 
					is_planar());
    else {
        ...
        try {
            for (y=0; y<view1.height(); ++y)
                detail::uninitialized_copy_aux (view1.row_begin(y),
						view1.row_end(y),
						view2.row_begin(y),
						is_planar());
        } catch(...) {
            for (typename View1::y_coord_t y0=0; y0<y; ++y0)
                detail::destruct_aux (view2.row_begin (y0),
				      view2.row_end (y0),
				      is_planar());
            throw;
        }
...
/**
   ...
   pixel iterator that does not check for the end of rows.  For
   non-contiguous images for_each_pixel resolves to for_each of each
   row using the underlying pixel iterator, which is still faster
...
/**
   \defgroup ImageViewSTLAlgorithmsForEachPixelPosition
   for_each_pixel_position
   
   \ingroup ImageViewSTLAlgorithms
   \brief adobe::for_each_position for image views (passes locators,
   instead of pixel references, to the function object)
...
/**
   ...
   PSYNTH pixels are bitwise comparable, so memcmp is used. User-defined
   pixels that are not bitwise comparable need to provide an overload
*/
template<typename T, typename Cs>
struct equal_n_fn<const pixel<T,Cs>*, const pixel<T,Cs>*>
{
    PSYNTH_FORCEINLINE
    bool operator () (const pixel<T,Cs>* i1, std::ptrdiff_t n,
...
/**
   ...
   type. memcmp is invoked for each channel plane
   User-defined channels that are not bitwise comparable need to
   provide an overload
*/
...
{
    PSYNTH_FORCEINLINE
    bool operator () (const planar_pixel_iterator<IC,Cs> i1,
		      std::ptrdiff_t n,
...
// IL Models ConstPixelLocatorConcept, O Models PixelIteratorConcept
struct equal_n_fn<psynth::sound::iterator_from_2d<Loc>,I2>
{
    PSYNTH_FORCEINLINE
...
// I Models PixelIteratorConcept, OL Models PixelLocatorConcept
struct equal_n_fn<I1,psynth::sound::iterator_from_2d<Loc> >
{
    PSYNTH_FORCEINLINE
...
struct equal_n_fn <psynth::sound::iterator_from_2d<Loc1>,
		   psynth::sound::iterator_from_2d<Loc2> >
{
   PSYNTH_FORCEINLINE
   bool operator () (psynth::sound::iterator_from_2d<Loc1> i1,
		     std::ptrdiff_t n,
...
/**
   ...
   end of each row) it is more efficient to use the underlying pixel
   iterator that does not check for the end of rows. If the underlying
   pixel iterator happens to be a fundamental planar/interleaved
   pointer, the call may further resolve to memcmp. Otherwise it
   resolves to copying each row using the underlying pixel iterator.
*/
template <typename Loc1, typename Loc2> PSYNTH_FORCEINLINE 
bool equal (psynth::sound::iterator_from_2d<Loc1> first,
	    ...
	    psynth::sound::iterator_from_2d<Loc2> first2)
{
    ...
    if (first.is_1d_traversable()) {
        if (first2.is_1d_traversable())
            return psynth::sound::detail::equal_n_fn <
		typename Loc1::x_iterator,
        ...
        if (first2.is_1d_traversable())
            return psynth::sound::detail::equal_n_fn<
		psynth::sound::iterator_from_2d<Loc1>,
		typename Loc2::x_iterator>() (
		    first, n, first2.x());
        else
            return psynth::sound::detail::equal_n_fn<
...
bool equal_pixels (const View1& v1, const View2& v2)
{
    ...
    // std::equal has overloads with PSYNTH iterators for optimal performance
}
...
F transform_pixels (const View1& src, const View2& dst, F fun)
{
    ...
    {
        typename View1::x_iterator src_it = src.row_begin(y);
        typename View2::x_iterator dst_it = dst.row_begin(y);
        for (std::ptrdiff_t x = 0; x < src.width(); ++x)
            dst_it [x] = fun (src_it [x]);
    }
    return fun;
}
...
PSYNTH_FORCEINLINE 
F transform_pixels (const View1& src1, const View2& src2,
		    const View3& dst, F fun)
{
    for (std::ptrdiff_t y=0; y<dst.height(); ++y)
    {
        typename View1::x_iterator src_it1 = src1.row_begin (y);
        typename View2::x_iterator src_it2 = src2.row_begin (y);
        typename View3::x_iterator dst_it  = dst.row_begin (y);
        for (std::ptrdiff_t x = 0; x < dst.width(); ++x)
            dst_it [x] = fun (src_it1 [x], srcIt2 [x]);
    }
    return fun;
}
...
/**
   ...
   \brief Like transform_pixels but passes to the function object
   pixel locators instead of pixel references
*/ 
template <typename View1, typename View2, typename F> PSYNTH_FORCEINLINE 
...
template <typename View1, typename View2, typename View3, typename F>
PSYNTH_FORCEINLINE 
F transform_pixel_positions (const View1& src1,
			     const View2& src2,
			     const View3& dst, F fun)
{
