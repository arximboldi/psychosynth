...
#include "synth/multi_point_envelope.hpp"
			       ...
			       int caller_port)
{
    ...
    sample* output = (sample*) &range (*outbuf) [0];
    const sample_buffer* bpmbuf = get_input<sample_buffer> (LINK_CONTROL, IN_C_BPM);
    const sample* bpm = bpmbuf ? (const sample*) &const_range (*bpmbuf) [0] : 0;
    size_t i;
...
void node_step_seq::init_envelope_values ()
{
    ...
    m_lo_env_vals[0] = synth::envelope_point<audio_sample> { 0.0f, 0.0f };
...
void node_step_seq::update_shape ()
{
    switch (m_param_shape) {
    case SHAPE_SQUARE:
	m_hi_env_vals[1] = {m_param_slope * m_param_high, 1.0f};
	m_hi_env_vals[2] = {m_param_high - m_param_slope * m_param_high, 1.0f};
	m_hi_env_vals[3] = {m_param_high, 0.0f};
	break;
    case SHAPE_TRIANGLE:
	m_hi_env_vals[1] = {m_param_high/2.0f, 1.0f};
	m_hi_env_vals[2] = {m_param_high, 0.0f};
	break;
    case SHAPE_FWSAWTOOTH:
	m_hi_env_vals[1] = {m_param_high - m_param_slope * m_param_high, 1.0f};
	m_hi_env_vals[2] = {m_param_high, 0.0f};
	break;
    case SHAPE_BWSAWTOOTH:
	m_hi_env_vals[1] = {m_param_slope * m_param_high, 1.0f};
