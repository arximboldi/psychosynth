...
public:
     ...
     */
    ring_audio_buffer(const audio_info& info, size_t size);
     ...
     */
    const audio_info& get_audio_info() const {
	return m_info;
    }
     ...
     */
    void set_info (const audio_info& info, size_t new_size) {
	if (info.num_channels != m_info.num_channels
	    || m_size != new_size) {
	    liberate();
	    m_size = new_size;
     ...
     */
    size_t read (read_ptr& r, audio_buffer& buf) const {
     ...
     */
    size_t read (read_ptr& r, audio_buffer& buf, size_t samples) const;
     ...
     * @param samples The number of samples to write.
     */
    void deinterleave (const sample* buf, size_t samples);
     ...
     * @param samples The number of samples to write.
     */
    void write (const audio_buffer& buf, size_t samples);
     ...
     * @param scaler The scaler to apply to the data.
     */
    void write_scaler (const audio_buffer& buf, size_t samples, scaler& scaler);
     ...
     * @todo This function is not backwards safe yet.
     */
    void write_fast_resample(const audio_buffer& buf, size_t samples, float factor);
