...
ring_sample_buffer::ring_sample_buffer (size_t size) :
    ring_buffer(size),
    m_data(NULL),
    m_fr_count(0)
{
...
size_t ring_sample_buffer::read(read_ptr& r, sample_buffer& buf, size_t samples) const
{
...
void ring_sample_buffer::write(const sample_buffer& buf, size_t nwrite)
{
    size_t offset = 0;
    if (nwrite > m_size) {
...
void ring_sample_buffer::write_fast_resample(const sample_buffer& buf, size_t samples, float factor)
{
    size_t i;
    size_t real_samples = ceil(samples * (1.0f / factor));
	...
	if (m_writepos + real_samples > m_size) {
		...
		m_data[i] = buf.get_data ()[(size_t)m_fr_count];
