...
have a significant \emph{decay time}\index{decay (envelope)} ---
...
The \type{processor} class is a \index{facade (design pattern)}
...
\index{forwarding port}\emph{forwarding ports}, which allow
...
\subsection{Polyphonic nodes basic interface}

\index{polyphony}While a full implementation of polyphonic audio will
...
The \type{poly\_node<Deriv>} template class is the common base of all
polyphonic nodes. It uses the ``Curiously Recurring Template Pattern''
\index{curiously recurring template pattern}, first documented by
...
intervene.\index{voice (polyphony)}
\begin{enumerate}
...
\item Objects that keep both some per-voice state and some common
  ...
  all voices and should not be repeated. The \type{poly\_*\_port}
...
\item State that changes with each voice. A nested \type{voice} class
  stores this state and its instances are managed transparently by the
  \type{poly\_node<>} class. An example of a per-voice state is the
  current oscillator position. This \emph{voice} class has a
  \type{rt\_do\_process} method similar to that in normal nodes;
...
of this chapter. In fact, only implementing the \type{poly\_*\_port}
and \type{poly\_node<>} classes, that should specialise their non
...
\scriptsize\label{note:mutrace-output}
\begin{verbatim}
...
...
was released at the end of this iteration. It was \index{peer-review}
...
\ref{note:changelog021}.\index{changelog} Note that Ubuntu packages
...
\begin{mynote}[Changelog of Psychosynth 0.2.1]
