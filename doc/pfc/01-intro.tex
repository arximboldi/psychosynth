...
\markboth{Preface}{A personal, historical and audiophile dissertation}
...
with some \emph{musíque concrete}\index{musique concrete}, then moving
...
\begin{mynote}[Kraftwerk and the first analog sequencers]\index{sequencer}
  In figure \ref{fig:kraftwerk} can see an old analog sequencer, As
  opposed to a synthesiser\index{synthesiser} --- this is, a virtual
  instrument\index{instrument}, which is in charge of
...
\begin{mynote}[Autechre and generative music]\index{generative music}
  ...
  Max/MSP\index{Max/MSP} and its Free Software counterpart Pure
  Data\index{PD, Pure Data}
  are graphical \emph{data-flow programming software}\index{data-flow
    programming}, which can be considered a sort of general low-level
...
The project won a price in that Free Software contest and it got some
...
Our problem definition is well contained in the title of this project:
...
A modular synthesiser\index{modular synthesiser} is one where the
...
\emph{generator}\index{generator}. A node with one input and one
output is often called a \emph{filter}\index{filter}. A node can have
different \emph{parameters}\index{parameter (synthesis module)} that
...
that are called \emph{modulators}\index{modulator}. A concrete
interconnection of a set of modules is commonly referred as a
\emph{patch}\index{patch}. Note \ref{note:modsynth} lists some of the
...
\begin{mynote}[Standard modules in an analog modular synthresizer]
  ...
  \begin{description}
  \item[VCO] \index{VCO, voltage controlled oscillator} Voltage
  ...
  \item[Noise source] \index{noise source} A generator that supplies
  ...
  \item[VCF] \index{VCF, voltage controlled filter} Voltage Controlled
  ...
  \item[EG] \index{EG, envelope generator} Triggering an Envelope
    ...
    sometimes available.
  \item[LFO] \index{LFO, low frequency oscillator} A Low Frequency
  ...
  \item[RM] \index{RM, ring modulator} Ring modulator, two audio
    ...
    ``robotic'' quality.
  \item[Mixer] \index{mixer} A module that combines multiple signals into one.
  \item[S\&H] \index{S\&H, sample and hold} Sample and hold, which
    ...
    loop.
  \item[Slew limiter] \index{slew limiter} Smooths off the peaks of
    voltages. This can be used to create glide or portamento between
    notes. Can also work as a primitive low-pass filter.
  \item[Custom Control Inputs] \index{custom control input} Because
...
Analog modular synthesisers\index{modular synthesiser!analog} where
invented in parallel 1968 by R. A. Moog Co. and Buchla in 1963
\cite{moog1964voltage}\index{Moog modular}. There, the sound signal is
...
ability to cope with \emph{polyphony}\index{polyphony}. We say that a
...
\emph{voice}\index{voice (polyphony)} --- and dispatch each keystroke to an
...
released during a \emph{decay time}\index{decay (envelope)} while the
sound softly fades out.

Nowadays, the increasing power of computers allows us to build modular
synthesisers by software\index{modular synthesiser!software}. Even
...
of his Moog\index{Moog modular} in the middle of his performances,
...
topology\index{topology} of the synthesis graph.

\subsubsection{Dynamic patching}

The \emph{dynamic patching}\index{dynamic patching}
\cite{kaltenbrunner04dynamic} technique was first introduced in the
ReacTable\index{Reactable} project and offers means to automatically
...
same kind. The sink node\index{sink node} that leads the output to the
...
\index{touchable interface}When striving for interactivity, an
...
There is an increasing availability of multi-touch
interfaces\index{multi-touch interface}, either specifically designed
...
\index{instrument}While touchable interfaces might be better for
...
MIDI\index{MIDI, musical instrument digital interface}
...
\index{collaborative environment}Since the beginning of times, music
...
\index{tangible interface}One approach to achieve this is by using a
...
physical space.  The ReacTable\index{Reactable} implements such an
...
\index{networking}Networking support releases a device even further
...
\index{framework}Music making and audio processing software and
...
following a Model-View-Controller\index{MVC, model-view-controller}
...
We are working in collaboration with the ArtQuimia\index{ArtQuimia}
...
\begin{objective}
  ...
  plug-ins\index{plug-in}, satisfying our own API and/or third-party
  standards.
\end{objective}
...
\emph{polyphony}\index{polyphony} as in most cases such feature would
...
\index{Moog modular}\index{modular synthesiser} The history of modular
audio generation starts with the Moog analog synthesiser in 1967
\cite{moog1964voltage}. Since then, a wide variety of analog modular
synthesisers have been developed commercially, but dragging some
limitations as described in the introductory section
\ref{sec:defmodular}.
...
this development is Max/MSP\index{Max/MSP} \cite{puckete2002max}. This
is a data-flow based visual programming\index{data-flow
  programming}\index{visual programming} environment mainly targeted
at music applications. In such software one can add boxes where one
types the name of the function that it should perform. When the name
has been written some connection plugs appear on its corners depending
on the module name, and one can draw lines connecting those
plugs. Figure \ref{fig:autechre} in the preface showed an example of
its functioning. The author of Max/MSP later developed a Free Software
counterpart called Pure Data \cite{puckette96puredata} that also has
video processing and synthesis support and is widely used nowadays.
...
Reaktor\index{Reaktor} (figure \ref{fig:reaktor}). In its version 5 it
...
Processing)\index{DSP, digital signal processing} programming, which
transparently compiled to efficient machine code. Later, Plogue's
Bidule\index{Bidule} is gaining special recognition for its simpler
interface. A new interesting software is XT Software's EnergyXT, a DAW
(Digital Audio Workstation)\index{DAW, digital audio workstation} that
...
view. Finally, Sensomusic's Usine\index{Usine} is remarkable for
...
On the Free Software\index{free software} side few modular
synthesisers exist. Alsa Modular Synth\index{Alsa Modular
  Synth}\footnote{\url{http://alsamodular.sourceforge.net/}} is one of
the most popular. Ingen\index{Ingen}
...
similar rival only in latest OSX versions: Jack\index{Jackd}
...
In the last decade, the development of touchable\index{multi-touch
  interface}\index{touchable interface}\index{tangible interface} and
...
improved interaction in musical software is the Audio Pad\index{Audio
  Pad} \cite{patten2002audiopad}, where the user places and moves
tagged pucks on a bidimensional surface. This is an example of an
interface with active tangibles\index{active tangible}, because the
...
project\index{Reactable} \cite{jorda07thereactable}. Its user
...
and rotation of passive tangibles\index{passive tangible}
...
ReacTIVIsion system on a computer. This system sends OSC\index{OSC,
  open sound control} (Open Sound Control) \cite{center03osc} based
messages to a synthesiser following the TUIO\index{TUIO, tangible user
  interface OSC} (Tangible User Interface OSC) protocol
...
Lemur\index{Lemur} and the Iwai-Yamaha’s Tenori-On\index{Tenori-On}
...
it the Indamixx 2 tablet\index{Indamixx tablet}, which is based on the
\index{Meego OS}Meego OS\footnote{Nokia and Intel's joint effort to
...
ToolKit)\index{STK, synthesis toolkit} \cite{scavone05rtmidi} but it
...
Language)\index{DSL, domain specific language} is
CSound\index{CSound}, which later was extended with a C and C++
...
SuperCollider\index{SuperCollider (DSL)}
\cite{mccartney2002supercollider}, which was for a long time the
reference sound synthesis DSL; Chuck\index{Chuck (DSL)}
\cite{wang03chuck}, that adds concurrency and time control in an
elegant and boilerplate-less fashion, and the newer Faust\index{Faust
  (DSL)} \cite{orlarey09faust}, which is based in a functional and
data-flow paradigm and interfaces easily due to its compilation to
C++\index{C++}\index{functional programming}.
...
\index{Impromptu} is a Scheme environment for live-coding, this is,
...
only. Its heavy modularity and dynamism is what make Lisp\index{Lisp}
dialects so interesting for live-coding and music composition. Common
Music\index{Common Music} \cite{taube97common} was started in 1989 and
...
CLAM (C++ Library for Audio and Music),\index{CLAM} an award winning
