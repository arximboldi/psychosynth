...
\index{analysis}Requirements \ref{req:iter1-begin} to
...
\index{signal (sound)}A sound signal is a longitudinal
wave\index{wave} of air in motion. We can analogically record the
\emph{proximal stimuli}\index{proximal stimuli} --- i.e. the physical
...
this measure. The \emph{amplitude}\index{amplitude} of this change is
associated to our perception of \emph{loudness}\index{loudness}, and
the frequency of this oscillation mostly logarithmically determines
our perception of \emph{pitch}\index{pitch}. We phrased this
...
\emph{decibels (dB)}\index{dB, decibel} above a standard reference
level.
...
\index{quantisation!temporal}Temporal quantisation relates to how many
...
\index{upsampling}\emph{upsample} a signal --- i.e. to figure out what
is between two samples. Most of the time we refer to the
\emph{sampling rate}\index{sample rate}, in hertz, as the frequency of
the temporal quantisation.\index{hertz}
...
We know from \emph{Niquist-Shannon sampling
  theorem}\index{Niquist-Shannon sampling theorem} that perfect
...
or equivalently, when the \index{Nyquist frequency} \emph{Nyquist
...
\index{quantisation!spatial}Spatial quantisation determines how many
...
\index{bitdepth}\emph{bitdepth} of 16 bit with samples that can take
...
arithmetic. The \index{dynamic range}\emph{dynamic range} of a signal
with $Q$-bits quantisation is:
\begin{equation}
  \mathrm{DR_{ADC}} = 20 \times \log_{10}(2^Q) = (6.02 \cdot Q)\, \mathrm{dB}
\end{equation}
\index{SNR, signal to noise ratio} The maximum \emph{signal-to-noise}
...
\emph{quantization error}\index{quantisation!error}, but the dynamic
...
\emph{dithering}\index{dithering}. Dithering is the process of adding
...
\index{channel}Because our hearing system is dicotomically
...
\emph{stereophonic}\index{stereophonic} sound, commonly named just
...
nowdays \emph{quadraphonic}\index{quadraphonic}, and \index{surround
  (audio)}\emph{surround} sound with varying numbers of channels up to
20.2 are used in different systems.

We call \emph{channel space}\index{channel!space} to the set of
semantic channels we use in some sort of audio representation ---
e.g. stereo sound has a channel space with $left$ and $right$
elements. We use the term \index{frame (audio)}\emph{frame} to call a
...
more accurate term \emph{frame rate}\index{frame rate} to characterise
...
\emph{interleaving}\index{interleaving} the samples of different
...
\emph{planar}\index{planar (non-interleaved)} to refer to
...
channels is stored. We call \index{channel
  layout}\emph{channel!layout} to a bijection $L : C \rightarrow
...
polymorphism\index{polymorphism!dynamic}, are simply not an option in
...
polymorphism\index{polymorphism!static}. This is the Generic Image
...
distribution. \index{Generic Image Library}
...
An \emph{image}\index{image} is bidimensional matrix of
\index{pixel}\emph{pixels}, that capture the properties of light
electromagnetic waveform at those discrete points. Each pixel,
however, is decomposed in several \index{colour}\emph{colours} that,
...
\index{design}\index{namespace}This section describes the design of
...
of static polymorphism and \index{generic programming}generic
...
\index{interface (OOP)}\index{concepts (C++)}\emph{Concepts}
...
\index{concept\_map}\texttt{concept\_map}. (5) Actually, the syntactic
...
similar to \index{Haskell}Haskell \emph{type classes}, with
\texttt{instance} doing the job of \texttt{concept\_map}
\cite{bernardy08comparison}.

Concepts are an extension to the template mechanism to add some sort
...
\index{metaprogramming}The C++ template system is Turing complete
...
Template metaprogramming is possible thanks to \emph{partial template
  specialisation}\index{template specialisation}, that allows giving
an alternate definition of a template for a pattern matched subset of
its possible parameter values. A
\emph{metafunction}\index{metafunction} is thus just a template
...
\index{meta-language}meta-language on its own, we should describe its
...
meta-functions (i.e. templates).\index{functional programming}
...
constant --- like \type{constexpr} in C++0x supporting
...
\emph{metafunction class}\index{metafunction class} as a type with a
...
metafunctions\index{high order metafunction} that take metafunction
...
A channel space\index{channel space} is a MPL sequence (like
\type{mpl::list}) of whose elements channel tags (empty types giving a
...
The \emph{layout}\index{layout} of a frame based type is a channel
...
A \emph{sample}\index{sample} is the type we use to represent the
...
A \emph{channel base}\index{channel base} is a container of channel
...
\type{bit\-aligned\_\-frame\_\-reference}, \type{packed\-\_frame} and so on.
...
to the underlying sample type with the \type{sample\_type}
metafunction.
...
\type{planar\-\_frame\-\_re\-fe\-rence} and
...
\type{FrameConcept}. These iterators must provide some extra
metafunctions, as in:
...
There exist a related \type{MutableIteratorAdaptor\-Con\-cept} with
the usual definition. Classes
\type{dereference\-\_iterator\-\_adaptor} and
...
\type{RangesAre\-Compati\-ble\-Concept}. Ranges are compatible if they
...
give mutable access to its referred frames, but a \type{const} buffer
...
\index{ring buffer}Circular or ring buffers provide a virtual
...
possible with the \emph{position} based interface --- the \type{read*}
and \type{write*} functions. When this is not possible, sometimes the
...
\index{STL, standard template library} \index{algorithm}The library
...
\index{polymorphism!dynamic}Until now, we have described a very
...
\index{disjoint union}\index{variant}For this purpose we use the
...
visitation function is \type{apply\_operation} in contrast with
Boost.Variant's \type{apply\_visitor}.

We provide the \type{variant} subclasses \type{dynamic\-\_buffer} and
\type{dynamic\-\_buffer\-\_range}. Note that the interface of these
types is more limited than the one of \type{buffer} and
\type{buffer\-\_range}. Specifically, they do not model
\type{RandomAccess\-Buffer} and \type{RandomAccessBufferRange} nor
...
provide an \type{operator()} overload for every format from the
...
\index{I/O, input and output} When talking about input and output,
...
version of UML \cite{umlspec}.\index{UML, unified modelling language}
...
\index{RAII, resource adquisition is initialisation} Note that none of
...
be used for opening several files, one can use \type{swap} or the
\emph{move constructor}. If the lifetime of variable does not match
the one of the resource \type{boost::optional} or a smart pointer
should be used.

\subsubsection{Buffering}

\index{buffering}For the reader who paid attention to our discussion
...
their \type{Range} parameter to model
\type{FrameBufferRange\-Concept}. The most sensible solution is to use
...
device. All the descendants of \type{async\_base} are I/O systems of
this kind. That interface provides \type{start ()} and \type{stop ()}
methods for controlling the state of the asynchronous operation, plus
others for checking its current status.\index{asynchronous (I/O)}

Whenever new data is sent or required by the device, a user specified
callback\index{callback} is executed with the number of frames as
parameter, which should in turn call \type{put ()} or \type{take ()}
...
to get real-time priority if requested\footnote{The \type{std::thread}
  ...
  \type{native\_handle} hook to request the new priority using the
  \type{pthreads} API directly when POSIX threads is the underlying
...
all the I/O devices based on the \type{thread\_async}
...
\index{caching (I/O)}As we have noted, system calls should be avoided
...
\index{unit test}Unit testing serves to ensure that the actual
...
The modules described in this chapter are evaluated with a total of
...
\type{BOOST\_UNIT\_TEST\_TEMPLATE} one can define a test case
...
The system was then \emph{peer reviewed}\index{peer review} by project
...
\ref{note:changelog02}. \index{changelog}
...
use of exceptions and \emph{scope guards}\index{scope guard}
\cite{alexandrescu00gener}.

Even though we believe that the benefits outweight the drawbacks, we
...
\index{lock-free (algorithm)}The audio processing thread has real time
...
higher priority, because \index{priority inversion}\emph{priority
...
Locking is done with care and in practice we haven't experienced any
buffer underrun\index{buffer underrun} due to this problem, even with
...
\index{SIMD, single instruction multiple data}
This is not a dead end. Using the \index{expression template}
...
\index{Boost library}In our conversations with Lubomir Bourdev he
